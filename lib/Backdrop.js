	...
	$( function () {
		...
		if ( $backDropRoot.length ) {
			...
			if ( $backgroundURL !== undefined ) {
				...
				$( '.mw-header' ).css( { 'width': '100vw', 'padding': '1rem 5rem 0 0', 'position': 'fixed' } );
			...
			if ( $hasCover !== undefined ) {
				...
				'position': 'fixed', 'bottom': '10rem', 'justify-content': 'space-between' } );
				
				var $titleGroup = $( '<div></div>' );
				$titleGroup.css( { 'display': 'flex', 'flex-direction': 'column', 'float': 'left', 'position': 'absolute',
				'margin-left': '1rem', 'color': $titleColor } );
			...
			'color': $textColor, 'margin-left': '15rem', 'z-index': '-1' } );
		}
	} );
