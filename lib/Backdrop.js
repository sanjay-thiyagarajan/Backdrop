	...
	$( function () {
		...
		if ( $backDropRoot.length ) {
			...
			if ( $hasCover !== undefined ) {
				...
				$titleAndLogoDiv.css( { 'display': 'flex', 'flex-direction': 'row', 'width': 'inherit',
				'position': 'absolute', 'bottom': 0, 'align-items': 'stretch' } );

				var $newTitle = $( '<p></p>' );
				$newTitle.text( $pageTitle );
				var $modifiedStyle = $.extend( {}, $titleStyle );
				$modifiedStyle.float = 'left';
				$modifiedStyle.width = '35%';
				...
				if ( $pageLogoField !== undefined ) {
					...
					$pageLogo.css( { 'max-width': '15rem', 'max-height': '5rem', 'display': 'flex',
