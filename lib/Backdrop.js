	...
	$( function () {
		...
		if ( $backDropRoot.length ) {
			...
			if ( $backgroundURL !== undefined ) {
				...
				$( '.mw-header' ).css( { 'width': '100vw', 'padding-top': '3rem', 'position': 'fixed', 'margin-top': '-1.5rem',
				'height': 'auto', 'background': 'url("' + $backgroundURL + '") no-repeat fixed center',
				'background-size': 'cover' } );
				$( '#content' ).css( { 'width': '100vw', 'padding': '0 2rem 0 1rem' } );
				$( '.mw-footer-container' ).css( { 'display': 'none' } );
				$( '#mw-panel' ).css( { 'position': 'fixed', 'z-index': 3, 'margin-top': '8rem' } );
			}
			...
			if ( $hideControl !== undefined && $currentUsername === null ) {
				...
				$( '#p-search' ).css( { 'align-self': 'flex-end', 'position': 'absolute', 'right': '3rem', 'margin-top': '1rem' } );
			}
			...
			if ( $hasCover !== undefined ) {
				...
				'position': 'fixed', 'bottom': '0.1px', 'justify-content': 'space-between', 'z-index': '999',
				...
				$( window ).scroll( function() {
					if ( $coverDiv.isInViewport() ) {
						...
						'margin-top': '60rem' } );
						$( '.sidebar-toc' ).css( { 'border-radius': '2rem', 'margin-top': '8rem', 'position': 'absolute' } );
					}
				} );
			} else {
